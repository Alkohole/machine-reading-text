...
/*:
 * @plugindesc (v1.5 *) Novas transições de batalha.
 * @author Moghunter
 *
 * @param Default Transition
 * @desc Definição da transição padrão.
 * (-1 = Default) (0..3 New Trasitions) (4 = Random) 
 * @default 4
 * @type select
 * @option Default
 * @value -1
 ...
 * - (BUG FIX) - Correção de ativar o comando inicial com o plugin
 *               MOG_EmergeMotion. 
 *  
 */
...
Scene_Base.prototype.removeTRSprites = function() {
	...
	if (Imported.MOG_EmergeMotion) {$gameTemp._emergingInt = 30};
};
...
Scene_Base.prototype.setupTRShatter = function() {	    
	 ...
	 for (var i = 0; i >= 0; i++) {
		 ...
		 this.addChild(this._spriteTrasition[i]);
		 if (this._spriteTrasition[i].y > this._bitmapTransition.height) {break};
	 };
	 for (var i = 0; i < 5; i++) {
...
Scene_Battle.prototype.create = function() {
	...
	if (!BattleManager.isBattleTest() && $gameSystem._treType[0] >= 0) {
        $gameSystem._trefctData[0] = true;
